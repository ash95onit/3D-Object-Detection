...
// Struct to output relevant data for evaluation
struct outputdata {
...
int main(int argc, const char *argv[])
{
    ...
    // Performance evalulation variables
    std::vector<outputdata> combos;
    // std::vector<std::string> keypoints{"SHITOMASI", "HARRIS", "FAST", "BRISK", "ORB", "AKAZE","SIFT" };
    std::vector<std::string> keypoints{"ORB" };
    // std::vector<std::string> descriptors{"BRIEF", "ORB", "FREAK", "AKAZE", "SIFT"};
    ...
    output_stream << "Detector type" << "," << "Desriptor type" << ","  << "Frame_ID" << ","
                 << "Camera TTC (s)" << ","  << "Lidar TTC (s)" << ","  << "Difference TTC" << std::endl;
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        try{
            ...
            std::cout << "CV Exception for kpt" << kpt << "and desc" << desc << std::endl;
            continue;
        }
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    outdata.detectorType = kpt;
                    outdata.descriptorType = desc;
                    outdata.frame_id = imgIndex;
                    outdata.ttc_camera = ttcCamera;
                    outdata.ttc_lidar = ttcLidar;
                    outdata.diff_ttc_camera_lidar = ttcCamera - ttcLidar;

                    combos.push_back(outdata);
    ...
    }} // eof detector and descriptor loop
